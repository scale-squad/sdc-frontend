...
const RatingGraph = ({ ratings, width = 270, setStarFilter, starFilter }) => {
  ...
  const allFiltersEnabled = starFilter.every(Boolean);
  const totalCount =
    Object
      .values(ratings)
  ...
  const toggleSelectedStar = (starNumber) => {
    ...
    if (allFiltersEnabled) {
      newStarFilter = new Array(5).fill(false);
    } else {
      newStarFilter = starFilter.slice();
    }
  ...
  return (<div>
    ...
    {
      starFilter.map((x, i) => !allFiltersEnabled && x ? <div>{i + 1} stars Filter enabled </div> : "")
    }
    {
      starFilter.some(x => x === false) ?
